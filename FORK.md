...
Some human messages are shared work for BOTH resident bots (burg1 = this plugin, burg2 = the Go daemon). Detection (`isSharedRequest` — the final agreed reference spec, mirrored by burg2's Go side) uses discord.js's **parsed** mentions (`msg.mentions.users`/`.roles`/`.repliedUser`, never content text, so code-formatted "pings" can't spoof it), bot authors never trigger (checked first), precedence highest first: (1) `@burgers` role ping OR both bots pinged → shared, outranks everything; (2) exactly ONE bot individually pinged → NOT shared, even in #ai-bot or #burg; (3) message is a reply to either burg (`msg.mentions.repliedUser` is either bot id) with no team ping → NOT shared; (4) human message in #ai-bot (`1120322640826077334`) or #burg (`1523307485786673302`) → shared; (5) otherwise → not shared.
